...
const Card = ({title, description, link, image ,github} : {title: string, description: string, link: string, image: string,github: string}) => {
  return (
      ...
      <CardBody className="bg-black bg-opacity-50 dark:bg-opacity-90 relative group/card border-neutral-700 w-auto sm:w-[30rem] h-auto rounded-xl p-6 border shadow-lg hover:shadow-[0_0_20px_5px_rgba(128,0,128,0.7)] transition-shadow duration-300">
        ...
        <div className="flex justify-between items-center mt-20">
          <CardItem
            ...
            href={link}
            target="__blank"
            className="px-4 py-2 rounded-xl text-xs font-normal dark:text-white text-[#8A2BE2] bg-transparent dark:bg-transparent hover:text-purple-500 transition-colors duration-300"
          >
            Check it Out→
          </CardItem>
          <CardItem
